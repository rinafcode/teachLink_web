...
const logger = createLogger('conference-end-session-api');

/**
 * POST /api/conference/meetings/{meetingId}/end
 *
...
export async function POST(
  ...
  if (authError) {
    logger.warn('Meeting end attempt without auth');
    return authError;
  }
  ...
  try {
    ...
    const meetingCheck = await query(
      `SELECT id, host_id, status FROM meetings WHERE id = $1`,
      [meetingId],
    );
    ...
    if (meeting.host_id !== requesterId) {
      logger.warn('Unauthorized meeting end attempt', {
    ...
    if (meeting.status === 'ended') {
      logger.warn('Meeting already ended', { context: { meetingId } });

      return NextResponse.json({ error: 'Meeting already ended' }, { status: 400 });
    }

    // End meeting
    const result = await query(
      `UPDATE meetings
       SET status = 'ended', recording_enabled = false, ended_at = NOW(), updated_at = NOW()
       WHERE id = $1
       RETURNING id, room_id, host_id, title, status, recording_enabled, created_at, started_at, ended_at, updated_at`,
      [meetingId],
    );

    const updatedMeeting = result.rows[0];

    // T8 MITIGATION: Log successful end
    appendAuditLog({
      actorId: requesterId,
      action: 'update',
      targetType: 'meeting',
      ...
      userAgent: request.headers.get('user-agent') || 'unknown',
      statusCode: 200,
      metadata: { meeting_ended: true },
    });

    logger.info('Meeting ended successfully', { context: { meetingId } });
    ...
    logger.error('Meeting end error', {
      context: { meetingId, requesterId },
      error,
    });
