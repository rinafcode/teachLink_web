...
const logger = createLogger('conference-start-recording-api');

/**
 * POST /api/conference/meetings/{meetingId}/start-recording
 *
...
export async function POST(
  ...
  if (authError) {
    logger.warn('Recording start attempt without auth');
    return authError;
  }
  ...
  try {
    ...
    if (meeting.host_id !== requesterId) {
      logger.warn('Unauthorized recording start attempt', {
    ...
    if (meeting.recording_enabled) {
      logger.warn('Recording already in progress', { context: { meetingId } });
    ...
    const result = await query(
      `UPDATE meetings
       SET recording_enabled = true, status = 'recording', updated_at = NOW()
       WHERE id = $1
    ...
    // T8 MITIGATION: Log successful start
    appendAuditLog({
      actorId: requesterId,
      action: 'update',
      targetType: 'meeting',
    ...
    logger.info('Recording started successfully', { context: { meetingId } });
    ...
    logger.error('Recording start error', {
      context: { meetingId, requesterId },
      error,
    });
