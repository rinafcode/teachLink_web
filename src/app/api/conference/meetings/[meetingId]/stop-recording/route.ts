...
const logger = createLogger('conference-stop-recording-api');

/**
 * POST /api/conference/meetings/{meetingId}/stop-recording
 *
...
export async function POST(
  ...
  if (authError) {
    logger.warn('Recording stop attempt without auth');
    return authError;
  }
  ...
  try {
    ...
    if (meeting.host_id !== requesterId) {
      logger.warn('Unauthorized recording stop attempt', {
    ...
    if (!meeting.recording_enabled) {
      logger.warn('Recording not in progress', { context: { meetingId } });

      return NextResponse.json({ error: 'Recording not in progress' }, { status: 400 });
    }

    // Stop recording
    const result = await query(
      `UPDATE meetings
       SET recording_enabled = false, status = 'active', updated_at = NOW()
       WHERE id = $1
    ...
    // T8 MITIGATION: Log successful stop
    appendAuditLog({
      actorId: requesterId,
      action: 'update',
      targetType: 'meeting',
    ...
    logger.info('Recording stopped successfully', { context: { meetingId } });
    ...
    logger.error('Recording stop error', {
      context: { meetingId, requesterId },
      error,
    });
