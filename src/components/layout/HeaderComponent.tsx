...
export default function HeaderComponent() {
  ...
  const activeScopesFormatted = useMemo(() => {
    ...
    return `${grantsState.grants.length} Scope ${grantsState.grants.length === 1 ? "Grant" : "Grants"} Authorized`;
  }, [grantsState.grants]);
    ...
    <header className="w-full bg-card border-b border-border text-card-foreground shadow-sm sticky top-0 z-50 transition-colors">
      <div className="max-w-7xl mx-auto h-16 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <Link href="/dashboard" className="flex items-center gap-2 font-extrabold text-xl tracking-tight text-foreground">
            <span className="bg-blue-600 text-white px-2 py-0.5 rounded text-sm font-mono">V3</span>
            StellarSplit
          </Link>
          <nav className="hidden md:flex items-center gap-4 text-sm font-medium text-muted-foreground">
            <Link href="/escrow" className="hover:text-foreground transition-colors">Escrows</Link>
        ...
        <div className="flex items-center gap-4">
          {connected && (
            <div className="relative">
              ...
              >
                <span className={`h-2 w-2 rounded-full ${grantsState.grants.length > 0 ? "bg-emerald-500 animate-pulse" : "bg-muted-foreground"}`} />
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-card border border-border rounded-xl shadow-lg p-4 space-y-3 z-50 text-sm">
                  ...
                  {grantsState.loading ? (
                    <div className="text-xs text-muted-foreground animate-pulse py-2 text-center">Syncing delegation keys...</div>
                    ...
                    <div className="max-h-60 overflow-y-auto divide-y divide-border space-y-2 pr-1">
                      {grantsState.grants.map((grant) => (
                        <div key={grant.id} className="pt-2 first:pt-0 flex items-start justify-between gap-2 text-xs">
