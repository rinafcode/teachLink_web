            ...
            {isOpen && (suggestions.length > 0 || history.length > 0) && (
                <div className="absolute top-full left-0 right-0 mt-3 glass-panel rounded-2xl border-slate-200 overflow-hidden shadow-2xl z-50 bg-white/98 animate-in fade-in slide-in-from-top-2 duration-300">
                            ...
                            <div>
                                ...
                                {suggestions.map((suggestion, i) => (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-colors text-sm text-left group ${activeIndex === i ? 'bg-primary/5 text-primary' : 'text-slate-600 hover:bg-slate-100'
                                            }`}
                                    >
                                        <div className={`${activeIndex === i ? 'text-primary' : 'text-slate-300 group-hover:text-primary'} transition-colors`}>
                                        ...
                                        <span className="flex-1 truncate">
                                            {highlightMatch(suggestion, value).map((part, index) => (
                                                <span
                                                    key={index}
                                                    className={part.toLowerCase() === value.toLowerCase() ? 'font-bold text-primary' : ''}
                                                >
                                                    {part}
                                                </span>
                                            ))}
                                        </span>
                                        <ChevronRight className={`w-3 h-3 transition-all ${activeIndex === i ? 'translate-x-0.5 opacity-100 text-primary' : 'opacity-0 group-hover:opacity-100 text-slate-200'
                                            }`} />
                                    </button>
                                ))}
                            </div>
                    ...
                    <div className="bg-slate-50/80 px-4 py-2 text-[10px] text-slate-400 flex justify-between border-t border-slate-100">
                        <span>Use <span className="p-0.5 bg-white border border-slate-200 rounded font-bold px-1">↓</span> <span className="p-0.5 bg-white border border-slate-200 rounded font-bold px-1">↑</span> to navigate</span>
