...
export function AdaptiveNavigation({ links, brandName = 'TeachLink' }: AdaptiveNavigationProps) {
  ...
  const handleDrawerKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.key === 'Tab') {
      const focusableElements = drawerRef.current?.querySelectorAll<HTMLAnchorElement>('a');
    ...
    <>
      <nav className="relative flex items-center justify-between p-4 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 z-40">
        ...
        <div className="hidden md:flex space-x-6 items-center">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-gray-600 hover:text-blue-600 dark:text-gray-300 dark:hover:text-blue-400 min-h-[44px] flex items-center font-medium transition-colors"
            >
              {link.label}
            </a>
          ))}
        ...
        <div
          ...
          className={`absolute top-full left-0 right-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 shadow-xl md:hidden flex flex-col p-4 space-y-3 transition-all duration-300 origin-top max-h-[calc(100vh-70px)] overflow-y-auto z-40
        ...
        >
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="p-4 text-lg font-semibold text-gray-800 dark:text-gray-100 bg-gray-50 dark:bg-gray-800/50 rounded-xl active:bg-gray-100 dark:active:bg-gray-800 active:scale-[0.98] transition-all touch-manipulation focus-visible:outline-2 focus-visible:outline-blue-500"
