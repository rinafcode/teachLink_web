...
const MessageBubble = memo(({ message, style }: MessageBubbleProps) => {
  const time = message.timestamp instanceof Date ? message.timestamp : new Date(message.timestamp);

  return (
    <div style={style} className={`message-wrapper ${message.isOwn ? 'message-wrapper--own' : ''}`}>
