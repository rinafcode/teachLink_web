...
export class FormConfigurationParser implements ConfigurationParser {
  ...
  formatToJson(config: FormConfiguration): string {
    try {
      ...
      throw new Error(
        `Failed to format configuration to JSON: ${
  ...
  formatToCompactJson(config: FormConfiguration): string {
    try {
      ...
      throw new Error(
        `Failed to format configuration to compact JSON: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  ...
  formatToJsonWithOptions(
    ...
    try {
      ...
      throw new Error(
        `Failed to format configuration with options: ${
          error instanceof Error ? error.message : 'Unknown error'
        }`,
      );
    }
  ...
  private validateAndTransform(parsed: unknown): FormConfiguration {
    ...
    if (customValidationErrors.length > 0) {
      throw new Error(
        `Configuration validation failed: ${customValidationErrors
          .map((e) => e.message)
          .join(', ')}`,
      );
    }
