  ...
  return (
    <div style={styles.container} suppressHydrationWarning>
      <div style={styles.header}>
        <h1 style={{ fontSize: "24px", fontWeight: "bold", margin: 0 }}>🔍 System Health & Monitoring Dashboard</h1>
      ...
      <div style={styles.section}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "10px", marginBottom: "15px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            ...
            <button onClick={clearLogs} style={styles.dangerButton}> Clear</button>
          </div>
        </div>
        ...
        <div style={styles.modalOverlay} onClick={() => setShowLogsModal(false)}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              ...
              <div>
                <button onClick={exportLogs} style={{ ...styles.buttonSmall, marginRight: "8px" }}> Export</button>
                <button onClick={clearLogs} style={styles.dangerButton}> Clear All</button>
                <button onClick={() => setShowLogsModal(false)} style={{ ...styles.closeButton, marginLeft: "8px" }}>Close ✕</button>
              </div>
            </div>
            <div style={{ overflowX: "auto", maxHeight: "60vh", overflowY: "auto" }}>
              <table style={styles.table}>
                <thead style={{ position: "sticky", top: 0, background: darkMode ? "#0f3460" : "#f0f0f0" }}>
                  <tr>
                  ...
                  {logs.length === 0 ? (
                    ...
                    logs.map((log, index) => (
                      <tr key={index} style={{ 
                        ...
                        background: log.event === 'failed' ? (darkMode ? "#3d1a1a" : "#fee2e2") : "transparent"
                      }}>
                        ...
                        <td style={{ ...styles.td, maxWidth: "250px", wordBreak: "break-word" }}>{log.message || "-"}</td>
                        ...
                        <td style={{ ...styles.td, color: "#ef4444", fontSize: "11px", maxWidth: "200px", wordBreak: "break-word" }}>
                          {log.error ? (
                            <details>
                              <summary style={{ cursor: "pointer", color: "#ef4444" }}> View Error</summary>
                              <pre style={{ marginTop: "8px", padding: "8px", background: darkMode ? "#2d2d2d" : "#f5f5f5", borderRadius: "4px", fontSize: "10px", whiteSpace: "pre-wrap" }}>
                                {log.error}
                              </pre>
                            </details>
                          ) : "-"}
            ...
            <div style={{ marginTop: "15px", padding: "10px", background: darkMode ? "#0f3460" : "#e5e7eb", borderRadius: "8px", fontSize: "12px", textAlign: "center" }}>
               Total: {logs.length} |  Sent: {logs.filter(l => l.event === 'sent').length} | 
               Delivered: {logs.filter(l => l.event === 'delivered').length} | 
               Clicked: {logs.filter(l => l.event === 'clicked').length} | 
               Failed: {logs.filter(l => l.event === 'failed').length}
            </div>
          </div>
        </div>
      )}
